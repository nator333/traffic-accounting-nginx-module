...
static void ngx_traffic_accounting_period_insert_value(
...
{
    ngx_rbtree_init(&period->rbtree, &period->sentinel, ngx_traffic_accounting_period_insert_value);
...
void ngx_traffic_accounting_period_insert_metrics(
    ngx_traffic_accounting_period_t *period, ngx_traffic_accounting_metrics_t *metrics
)
{
...
void ngx_traffic_accounting_period_delete_metrics(
    ngx_traffic_accounting_period_t *period, ngx_traffic_accounting_metrics_t *metrics
)
{
...
ngx_int_t ngx_traffic_accounting_period_rbtree_iterate(
    ngx_traffic_accounting_period_t *period, ngx_traffic_accounting_period_iterate_func func, void *para1, void *para2
)
{
    ngx_rbtree_t *rbtree;
    ngx_rbtree_node_t *node, *sentinel;
...
)
{
    ...
    for (;;) {
        ...
        if (node->key != temp->key) {
            ...
            p = (ngx_memcmp(n->name.data, t->name.data, n->name.len) < 0) ? &temp->left : &temp->right;
        }
