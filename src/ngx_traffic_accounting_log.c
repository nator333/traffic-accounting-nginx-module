...
)
{
    ...
    p = ngx_slprintf(
        p, last,
        "pid:%i|from:%i|to:%i|accounting_id:%V|%s:%ui|bytes_in:%ui|bytes_out:%ui|latency_ms:%ui|upstream_latency_ms:%"
        "ui",
        ngx_getpid(), created_at->sec, updated_at->sec, &metrics->name, entry_n, metrics->nr_entries, metrics->bytes_in,
        metrics->bytes_out, metrics->total_latency_ms, metrics->total_upstream_latency_ms
    );
    ...
    for (i = 0; i < statuses_len; i++) {
        ...
        p = ngx_slprintf(p, last, "|%i:%i", statuses[i], metrics->nr_status[i]);
    }
